indexing
	description: "Construct Non_optional_construct"
...
class NON_OPTIONAL_CONSTRUCT inherit

	S_NON_OPTIONAL_CONSTRUCT
		redefine
			post_action
		end;
...
feature {CONSTRUCT} -- semantics
	...
	post_action (level: INTEGER) is
		do
			info.working_production.set_latest_info_not_optional
			first.post_action (level)
		end -- post_action

end -- class NON_OPTIONAL_CONSTRUCT
