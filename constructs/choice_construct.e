indexing
	description: "Construct Choice_construct"
...
class CHOICE_CONSTRUCT inherit
...
feature {CONSTRUCTS} -- semantics
	...
	post_action (level: INTEGER) is
		...
		do
			from
				!! cs.make (info.productions_defined.last);
				info.set_working_production (cs);
				child_start
			until
				child_after
			loop
